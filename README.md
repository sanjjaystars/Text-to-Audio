...
- **Audio merging:** WAV chunk stitching with sample-rate validation + MP3 encoding via `lamejs`
...
Copy `.env.example` to `.env.local` and fill in the values. Keep real keys only in `.env.local` so they stay out of git.
...
| --- | --- | --- |
| `OPENAI_API_KEY` | Yes | API key for OpenAI text-to-speech generation. Do not commit a real key. |
...
5. The server requests TTS audio per chunk, validates the WAV chunk format, stitches the chunks together, and encodes a single MP3.
